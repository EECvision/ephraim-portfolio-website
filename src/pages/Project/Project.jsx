...
import { calcView, mapImageToId, toSentenceCase } from "../../components/utils";
...
const Project = ({ setCurrentPage, setMount }) => {
  ...
  const { center, back, close, projectContents, contentId, activeContentId } =
    state;
  ...
  useEffect(() => {
    try {
      ...
      document.getElementById("swipe").onanimationend = (e) => {
  ...
  useEffect(() => {
    ...
    document.getElementById("contentNav").style.opacity =
      (getScroll() / 100) * 1;
  ...
  return (
    ...
    >
      ...
      ></div>
      <nav
        style={activeContentId !== "0" ? { color: "black" } : {}}
        className={cx.nav}
      >
      ...
      <div className={`${cx.wrapper} ${close && cx.hide}`}>
        <div className={cx.fixed}>
          ...
          <div
            style={{
              backgroundImage: `url(${mapImageToId[Number(params.id) - 1]})`,
            }}
            ref={cardRef}
            className={`${cx.card} ${center && cx.center}`}
          ></div>
          <div
            ref={titleRef}
            className={`${cx.projectTitle} ${center && cx.center}`}
          >
            {toSentenceCase(projectTitle)}
          </div>
        </div>
