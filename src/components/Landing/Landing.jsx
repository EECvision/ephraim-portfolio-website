...
const Landing = ({ inView, setInview, view, setCurrentPage, m }) => {
  ...
  useEffect(() => {
    if (counter >= 4) return handleSetState({ counter: 3 });
  ...
  useEffect(() => {
    try {
      document.getElementById("projectInfo").onanimationend = () => {
  ...
  return (
    <div id="landing" style={{ background: background }} className={cx.landing}>
      ...
      <div className={cx.mobileProjectSelectorWrapper}>
        <MobileProjectSelector props={{ mainColor, id }} />
      </div>
      <div ref={projectWindowRef} className={cx.MobileProjectWindowWrapper}>
        <MobileProjectWindow />
      </div>
      <div
        id="projectInfo"
        className={`${cx.projectInfo} ${dir === "down" && cx.up} ${
          dir === "up" && cx.down
        }`}
      >
        <h1 style={{ color: mainColor }} className={cx.projectTitle}>
      ...
      <div className={cx.projectSelectorWrapper}>
        <ProjectSelector props={{ mainColor, id }} />
      </div>
